...
describe("ClustersView", () => {
  ...
  it("shows an error banner when the list call fails", async () => {
    vi.spyOn(global, "fetch").mockResolvedValue(
      jsonResponse({ error: { code: "store_error", message: "boom" } }, 500),
    );
    render(<ClustersView />);
  ...
  it("creates a cluster via the dialog", async () => {
    const spy = vi
      .spyOn(global, "fetch")
      .mockResolvedValueOnce(jsonResponse({ clusters: [] })) // initial list
  ...
  it("deletes a cluster", async () => {
    const spy = vi
      ...
      .mockResolvedValue(jsonResponse({ clusters: [] })); // refresh
    render(<ClustersView />);
